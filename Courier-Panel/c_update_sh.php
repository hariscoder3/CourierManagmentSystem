...
<?php
    ...
    if(isset($_POST['submit'])){
        ...
        $sql = "UPDATE `shipment` SET `delievery_date` = '$new_date', `delievery_status` = '$new_d_status', `amount_status` = '$new_amount_status' WHERE `shipment`.`sh_id` = $sh_id;";

        if($conn->query($sql) === TRUE) {

            if($new_d_status == 'Delievered'){

                $sql2 = "delete from assign where sh_id='$sh_id';";
                $sql3 = "delete from manages where sh_id='$sh_id';";
